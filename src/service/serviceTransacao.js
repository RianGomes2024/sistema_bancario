...
const transferir=(async(numero_conta,valor,cpf,descricao)=>{
    ...
    if(contaDestino==="desativada")throw new Error("Conta destino não encontrada!");
    if(contaOrigem==="desativada")throw new Error("Conta origem não encontrada");
    
    const id_conta_origem=contaOrigem.id_conta
    const id_cliente_origem=contaOrigem.id_cliente
    const numero_conta_origem=contaOrigem.numero_conta
    const saldoOrigem=Number(contaOrigem.saldo)
    const novaContaOrigem=new classConta(numero_conta_origem,id_cliente_origem,saldoOrigem)
...
const sacar=(async(valor,cpf,descricao)=>{
    ...
    if(verifyConta.status_conta==="desativada")throw new Error("ERRO! Conta não encontrada");
